...
// SEGMENTTREE template ends

ll seg_func(ll x, ll y) {
  return x + y;
}
...
void solve() {
	ll n, q; 
	cin >> n >> q;
	vector<ll> x(n), sum_v(n, 0);
	READ(x);
	SegmentTree<ll> sg(sum_v, 0, seg_func);
	TC(q) {
    ll k; cin >> k;
    if(k == 1) {
      ll a, b, c;
      cin >> a >> b >> c;
      --a, --b;
      ll left_val = sg.query(a, a);
      ll right_val = sg.query(b + 1, b + 1);
      sg.update(a, left_val + c);
      sg.update(b + 1, right_val - c);
    } else {
      cin >> k;
      --k;
      OUT(x[k] + sg.query(0, k));
    }
	}
}
