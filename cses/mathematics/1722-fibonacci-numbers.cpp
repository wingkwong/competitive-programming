...
mint fib(ll n) {
	// solution in O(n) would fail as n can be up to 10 ^ 18
	// we need to calculate fibonacci numbers in O(log n) 
	// ref: https://en.wikipedia.org/wiki/Fibonacci_number#Matrix_form
	if(f.count(n)) return f[n];
	if(n == 0) return 0;
	if(n == 1 || n == 2) return 1;
	ll k = (n & 1) ? (n + 1) / 2 : n / 2;
	return f[n] = (n & 1) ? (fib(k) * fib(k) + fib(k - 1) * fib(k - 1)) : ((mint(2) * fib(k - 1) + fib(k)) * fib(k));
}
