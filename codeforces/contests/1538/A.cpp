...
void solve() {
    int n; cin >> n;
    int mx = INT_MIN, mi = INT_MAX, mx_idx = 0, mi_idx = 0;
    REP(i, n) {
        int x; cin >> x;
        if(x > mx) {
            mx = x;
    ...
    if(mi_idx > mx_idx) swap(mi_idx, mx_idx);
    // int ans = min(
    //     min(
    //         max(n - mi_idx, n - mx_idx), 
    //         min(mi_idx + n - mx_idx + 1, mx_idx + n - mi_idx + 1)
    //     ),
    //     max(mi_idx + 1, mx_idx + 1)
    // );
    int ans = min({
        mx_idx + 1,
        mi_idx + 1 + n - mx_idx,
