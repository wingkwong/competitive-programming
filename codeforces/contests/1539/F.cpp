...
void solve() {
    ...
    segtree st(n + 1);
    REP(i, n) st.modify(i + 1, n, 1);
    int start = 0;
    while(start < n) {
        int end = start;
        while(end + 1 < n && a[order[end + 1]] == a[order[end]]) end++;
        FORN(j, start, end) {
            int i = order[j];
            auto L = st.get(0, i), R = st.get(i + 1, n);
            MAX(ans[i], ((R.mx - 1) - L.mn + 1) / 2);
        }
        FORN(j, start, end) st.modify(order[j] + 1, n, -2);
        FORN(j, start, end) {
            int i = order[j];
            auto L = st.get(0, i), R = st.get(i + 1, n);
            MAX(ans[i], (L.mx - (R.mn + 1)) / 2); 
        }
        start = end + 1;
    }
    output_vector(ans);
