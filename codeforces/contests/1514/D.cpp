...
const int NO_OF_RANDOM_ELEMENTS = 40;
const int mxN = 3e6 + 5;
...
void solve() {
    ...
    TC(q) {
        int l, r; cin >> l >> r;
        int ans = 1;
        REP(i, NO_OF_RANDOM_ELEMENTS) {
            int rand = uniform_int_distribution<int>(l, r)(rng);
            int x = a[rand];
            int occu = upper_bound(ALL(v[x]), r) - lower_bound(ALL(v[x]), l);
            int N = r - l + 1;
            // let N be thr range [l, r], i.e (r - l + 1)
            // (a) put all (N  - occu) non v[x] elements and (N - occu + 1) v[x] elements to ONE block
            // (b) put the remaining v[x] elements (occu - (N - occu + 1)) to each bock 
            //  
            // therefore, we got 
            // 1 + (occu - (N - occu + 1))
            // = 2 * occu - N
