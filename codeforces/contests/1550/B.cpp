...
void solve() {
    ...
    // ==> (a * l[i]) for i = 1 .. k  + b * k
    // ==> a * n + b * k
    // b >= 0 : delete one by one so that k = n 
    // block = 3 : 00 11 00
    // operation : floor(block / 2) + 1
    // block > 2 : remove one = remove two (after merging)
    int block = unique(ALL(s)) - s.begin();
    OUT((a * n + max(b * n, ((block / 2) + 1) * b)));
}
