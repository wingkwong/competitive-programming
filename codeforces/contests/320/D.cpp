...
void solve() {
    ...
    for(int ans = 0;;ans++) {
        vi killers, dead; // a[i]
        EACH(cur_candidate, candidates) {
            auto it = alive.find(cur_candidate);
            if(it == alive.end()) continue;
            auto next_candidate = next(it);
            if(next_candidate == alive.end()) continue;
            if(a[cur_candidate] > a[*next_candidate]) {
                killers.pb(cur_candidate);
                dead.pb(*next_candidate);
            }
        }
        if(SIZE(killers) == 0) {
            OUT(ans);
