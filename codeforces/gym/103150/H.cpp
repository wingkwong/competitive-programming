...
void solve() {
    int n; cin >> n;
    long double ans = 0.0;
    // E[A[i]] = 0.5 * E[A[i] - 1] + 0.5 * E[0] + 1
    // E[A[i]] = 1 + 0.5 * E[A[i] - 1]
    // E[A[i]] = 1 + 0.5 + 0.25 * E[A[i] - 2]
    // E[A[i]] = 1 + 0.5 + 0.25 + 0.125 + ... + 1 / 2 ^ (A[i] - 1)
    // Geometric Progression :
    // E[A[i]] = 1 * (1 - (1 / 2 ^ A[i])) / (1 - (1 / 2))
    // E[A[i]] = 2 - (1 / (2 ^ (A[i] - 1)))
    // Linearity of Expectation :
    // ans = sum of E[A[i]] for i = 0 .. N - 1
    REP(i, n) {
        ll x; cin >> x;
        ans += 2 - 1 / fastpow(2, x - 1);
    }
